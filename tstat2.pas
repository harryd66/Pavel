...
Const
  {приход}
   ...
   CPrihSSum=CPrihS+(CKol+2)+1;
      CPrihB=CPrihSSum+CIZena+1;
   CPrihBSum=CPrihB+(CKol+2)+1;
  {продажи}
...
Begin
 For cur:=0 To VarList^.List^.Count-1 Do
  Begin
   ...
   DInfoMsg('Создаю временные файлы для позиции '+artikul+'...',False);
...

{
 ...
 For cur:=0 To VarList^.List^.Count-1 Do
  Begin
   ...
   s:=Copy(s,1,CName+1+CArtikul+1);
         {приход клиент}             {приход склад}               {приход обмен}
   s:=s+'      0│           0.00│'+'      0│           0.00│'+'      0│           0.00│'+
         {отгрузка клиент}           {отгрузка склад}             {отгрузка обмен}
 ...
 For cur:=Lstart To LStop Do
  Begin
   DInfoMsg('Просматриваю операции за '+DateToDateString(DateMask,Cur)+'...',True);
...
CalcTWindow^.Insert(ControlRp);
 ...
 DInfoMsg('Удаляю временные файлы...',True);
...
DinfoMsg('Сортирую выборку...',False);
