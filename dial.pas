...
{ Color palettes }
{ Вариант с черными оконтовками в окнах
  ...
  begin
    case Event.What of
      ...
      evKeyDown:
        begin
          SaveState;
          Event.KeyCode := CtrlToArrow(Event.KeyCode);{!Убрал я чтобы проходили символы #1 #0 !}
...
(*
{новая функция с учетом исправленных багов}
procedure TCluster.SetButtonState(AMask: Longint; Enable: Boolean); assembler;
asm
        LES     DI,Self
        MOV     AX,AMask.Word[0]
...
if not Link^.Focus then
  begin
   ClearEvent(Event);
   Exit;
  end;                 {Высота и ширина окна истории}
 FindStrok[0] :=#0;
 FindSymbol := 0;
 Link^.GetBounds(R);
 Link^.GetData(RSLT);
 ...
 {История сохраняется только для калькулятора,для остальных очистка}
...
15:{Baz}
   Begin
      st:='\\Second\hp  │отдел прихода';
      HistoryAdd(HistoryId, st);
      st:='\\Shef1\Hp   │программисты';
      HistoryAdd(HistoryId, st);
      st:='\\Operator\Hp│торговый зал';
      HistoryAdd(HistoryId, st);
   End;

16:{Baz}
   Begin
      st:='@!RCI3!│Матричный принтер';
      HistoryAdd(HistoryId, st);
      {st:=#27+#69+#27+#38+#108+#54+#68+#27+#38+#107+#52+#83;}
       st:=#27+#69+#27+#38+#108+#55+#67+#27+#38+#107+#52+#83+'  │Лазерный принтер';{E&l7C&k4S1}
	  {E&l7C&k4S1}
      HistoryAdd(HistoryId, st);
   End;
...
New(Dlg, Init(R, 'Поиск по вхождению'));
...
R.Assign(53, 3, 80, 20);
New(ExamplDateWin, Init(R, 'Укажите дату'));
