...
While Not(Eof(ZF)) Do
 Begin
  DInfoMsgShkala('Просматриваю реестр заказов ...',0,Start,FilePos(ZF));
  New(E,Init);
  Count:=0;
...
If (P^.List<>Nil) And (P^.List^.Count>=1) Then
 ...
 If c<>0 Then
  Begin
        MessageBox(#3^m+#3+'Не могу создать файл '+Path^.Dat.ToTemp+'listz.txt',Nil,mfError+mfCancelButton);
   Exit;
  End;
...
If Not(SelectionAgent(Agent)) Then
 ...
 If RegimExt=1 Then Writeln(f,Space+'ТОЛЬКО АКЦИЗНЫЙ ТОВАР');
...
*)
 Writeln(f,Space+'┌──────────────────────────────────────────────────────────────────────────────────────────┐');
 Writeln(f,Space+'│Клиент              Заказ  Дата    Срок До  Сумма отг. Сумма скид.Вид докум. Оператор     │');
  ...
  Begin
        If Abs(Itogo[c])>=0.01 Then
         Begin
          ...
          4:ws:=ws+Space+' С/Ф Б:    '+RecognizReal(Itogo[c],CIZena,CMantissa)+' руб '+^M;
     ...
     8:ws:=ws+Space+' Дебит СФБ:'+RecognizReal(Itogo[c],CIZena,CMantissa)+' руб '+^M;
     Else ;
     End;
    End;
  End;
 ...
 Begin
  For c1:=0 To TempRegion^.List^.Count-1 Do
   Begin
    ...
    Writeln(f,Space+'┌──────────────────────────────────────────────────────────────────────────────────────────┐');
    Writeln(f,Space+'│Клиент              Заказ Сумма отг.  Масса,кг Фактический адрес достав.                  │');
    ...
    If (TempListZ^.List<>Nil) And (TempListZ^.List^.Count>=1) Then
     ...
     End;{TempListZ}
    Writeln(f,Space+'────────────────────────────────────────────────────────────────────────────────────────────');
    Writeln(f,Space+'Всего по региону точек : ',LocCount:3,' ',RecognizReal(StrToReal(LocSumma),CIZena,CMantissa)+
    ' руб  '+ RecognizReal(StrToReal(LocMassa),CPack,CLitrMantissa)+' кг ');
    Writeln(f,Space+'────────────────────────────────────────────────────────────────────────────────────────────');
    Writeln(f);
